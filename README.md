...
- **Mechanism:** Uses `run_redteam.py` to simulate many adversarial variants of Context Poisoning attacks, testing specifically for `IndirectInstruction` and `ExcessiveAgency` vulnerabilities across both the "naive" and "hardened" agents.
  - **Generation & Attack Calculation:** `deepteam` automatically synthesizes adversarial inputs. The total number of attacks is calculated by taking the number of vulnerability types (not vulnerability classes) multiplied by the number of attack types multiplied by the attacks_per_vulnerability_type configuration. IndirectInstruction and ExcessiveAgency look like 2 vulnerabilities, but DeepTeam expands each into sub-types (IndirectInstruction → 4, ExcessiveAgency → 3), for 7 vulnerability types total. With 1 attack type, total = 7 × 1 × attacks_per_vulnerability_type.
